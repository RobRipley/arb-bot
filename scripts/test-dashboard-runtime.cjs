...
(async()=>{
  ...
  ctx.latestRouteRuntime={...status,live_authorized:true,dry_run:false,scheduler_in_flight_since_ns:[BigInt(Date.now())*1000000n]};
  assert.equal(vm.runInContext('routeAutomationState().state',ctx),'On','stale cache with an active in-flight scheduler must not block a running quote batch');
  assert.equal(vm.runInContext('routeAutomationState().label',ctx),'Scanning');
